...
def image_callback(msg):
    ...
    try:
        ...
        #cap = cv.imdecode(np.frombuffer(msg.data, dtype=np.uint8), cv.IMREAD_COLOR)
        # Now you can use cv_image as a regular OpenCV image
        #cap = cv.imshow("Pi cam feed", cv_image)
        #cv.waitKey(0)
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                pub = rospy.Publisher('gesture_topic', Int64, queue_size=1)
                pub.publish(hand_sign_id)
